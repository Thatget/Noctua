...
class Result extends \Magento\Framework\Model\AbstractModel implements IdentityInterface
{
    ...
    public function sendEmail($recipient = 'admin', $contact = false)
    {
        ...
        //     $sender['email'] = $this->getEmailValue();
        // } else {
            ...
            foreach ($email_array as $email) {
                if (!is_null($email)) {
                    $this->_transportBuilder
                        ...
                        ->setFrom($sender)
                        ->addTo($email)
                        ->setReplyTo($this->getReplyTo($recipient), $this->getCustomerName())
                    ;
    ...
    public function getEmailSubject($recipient = 'admin')
    {
        ...
        $subject = __("%1 order", $webform_name);
    ...
    public function getCustomerName()
    {
        ...
        if (count($customer_name) == 0) {
            /**
             * Changes RH : for set customer first name and last name in webform mail
             */
            $field_connection = $this->_fieldFactory->create()
                ->getCollection()
                ->addFilter('webform_id', $this->getWebformId())
                ->addFieldToSelect(['name', 'code']);
            //$field_output = $_POST['field'];
            $field_output = $this->getData();

            $firstName_id = '';
            $lastName_id = '';
            $comany_data = '';
            foreach ($field_connection as $key => $value) {
                if ($value['code'] == "FirstName") {
                    $firstName_id = $value['id'];
                }
                if ($value['code'] == "LastName") {
                    $lastName_id = $value['id'];
                }if ($value['code'] == 'Company') {
                    $comany_data = $value['id'];
                }
            }
            /**
             * Changes RH : for set customer first name and last name in webform mail
            ...
            if ($firstName_id != '' && $lastName_id != '') {
                return (string) __($field_output['field_'.$firstName_id] . ' ' . $field_output['field_'.$lastName_id]);
            }if ($firstName_id == '' && $lastName_id == '' && $comany_data != '') {
                $val = $field_output['field_'.$comany_data];
                return (string) __(strval($val));
            }if ($firstName_id == '' && $lastName_id == '' && $comany_data == '') {
    ...
    public function getEmailValue()
    {
        ...
        $email_value = '';
        foreach ($field_connection as $key => $value) {
            if ($value['code'] == "E-Mail") {
                $email_id = $value['id'];
            }
        }
        if ($email_id != "") {
            $email_value = $field_output[$email_id];
        }
        return $email_value;
    }
    ...
    public function toHtml($recipient = 'admin', $options = [])
    {
        ...
        $head_html = "";
        if ($options['header']) {
            $head_html = htmlspecialchars_decode($html);
        }
                        ...
                        if (!in_array($field->getType(), $options['skip_fields']) && $field->getResultDisplay() != 'off') {
                            ...
                            switch ($field->getType()) {
                                ...
                                case 'select/contact':
                                    ...
                                    break;
                                case 'html':
                                    $value = trim(htmlspecialchars_decode($field->getValue('html')));
                                    break;
                                case 'wysiwyg':
                                    $value = trim($value);
                                    break;
                                ...
                                case 'subscribe':
                                    ...
                                    break;
                                default:
                                    //$value = nl2br(htmlspecialchars_decode($value));

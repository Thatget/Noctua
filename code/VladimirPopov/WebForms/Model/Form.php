...
class Form extends AbstractModel implements IdentityInterface
{
    ...
    public function _getLogicTarget($uid = false)
    {
        ...
        if ($uid) {
            ...
            foreach ($this->_logic_target as $target) {
                if (strstr($target['id'], 'field_')) {
                    $target['id'] = str_replace('field_', 'field_' . $uid, $target['id']);
                }

                if (strstr($target['id'], 'fieldset_')) {
                    $target['id'] = str_replace('fieldset_', 'fieldset_' . $uid, $target['id']);
                }

                if (strstr($target['id'], 'submit')) {
                    $target['id'] = str_replace('submit', 'submit' . $uid, $target['id']);
                }
    ...
    {
        if ($this->getData('dashboard_groups')) {
            return $this->getData('dashboard_groups');
        }
    ...
    {
        if ($this->getData('access_groups')) {
            return $this->getData('access_groups');
        }
    ...
    public function validatePostResult()
    {
        ...
        if ($this->_registry->registry('webforms_errors_flag_' . $this->getId())) {
            return $this->_registry->registry('webforms_errors_' . $this->getId());
        }

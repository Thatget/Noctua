...
class Field extends AbstractModel implements IdentityInterface
{
    ...
    public function getSizeValues($empty_option = false)
    {
        ...
        if ($empty_option) {
            $sizes = array_merge([false => __('Not set')], $sizes);
        }
    ...
    {
        if ($this->getConfig('field_value')) {
            return $this->getConfig('field_value');
        }
        ...
        switch ($field_type) {
            ...
            case 'number':
                ...
                if (!empty($field_value['number_min'])) {
                    $field_value['number_min'] == '0' ? $config['min'] = 0 : $config['min'] = $field_value['number_min'];
                }

                if (!empty($field_value['number_max'])) {
                    $field_value['number_max'] == '0' ? $config['max'] = 0 : $config['max'] = $field_value['number_max'];
                }
            ...
            case 'textarea':
                ...
                if (!$customer_value) {
                    empty($field_value['textarea']) ? $config['field_value'] = '' : $config['field_value'] = $field_value['textarea'];
                }
...
<script>
    ...
    {
        if ($this->getData('result_label')) {
            return $this->getData('result_label');
        }

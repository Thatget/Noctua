...
class SliderEavTablesSetup {
	...
	/**
	 * pass value for create eav attribute vise table structure in createEAVMainTable and createEntityTable function
	 */
	public function createEavTables($entityCode) {
		$this->createSliderEAVMainTable($entityCode);
		$this->createSliderEntityTable($entityCode, 'datetime', Table::TYPE_DATETIME);
		$this->createSliderEntityTable($entityCode, 'int', Table::TYPE_INTEGER);
		$this->createSliderEntityTable($entityCode, 'text', Table::TYPE_TEXT, '64k');
		$this->createSliderEntityTable($entityCode, 'varchar', Table::TYPE_TEXT, 255);
	}

	/**
	 * create eav attribute table structure
	 */
	protected function createSliderEAVMainTable($entityCode) {

		$tableName = 'mediarocks_proslider_slider_eav_attribute';
		...
		$table = $this->setup->getConnection()->newTable(
			...
			'Mediarocks ProSlider Slider Eav Attribute'
		);
	 ...
	 */
	protected function createSliderEntityTable($entityCode, $type, $valueType, $valueLength = null) {
		...
		$table = $this->setup->getConnection()
			...
			->addIndex(
				$this->setup->getIdxName(
					$tableName,
					['entity_id', 'entity_type_id', 'attribute_id', 'store_id'],
			...
			->setComment('Mediarocks ProSlider Slider ' . $type . 'Attribute Backend Table');
		$this->setup->getConnection()->createTable($table);
	}
}

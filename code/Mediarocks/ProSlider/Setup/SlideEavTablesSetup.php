...
class SlideEavTablesSetup {
	 ...
	 */
	public function createSlideEavTables($entityCode) {
		$this->createSlideEAVMainTable($entityCode);
		$this->createSlideEntityTable($entityCode, 'datetime', Table::TYPE_DATETIME);
		$this->createSlideEntityTable($entityCode, 'int', Table::TYPE_INTEGER);
		$this->createSlideEntityTable($entityCode, 'text', Table::TYPE_TEXT, '64k');
		$this->createSlideEntityTable($entityCode, 'varchar', Table::TYPE_TEXT, 255);
	}

	/**
	 * create eav attributes tables and add foreign keys
	 */
	protected function createSlideEAVMainTable($entityCode) {

		$tableName = 'mediarocks_proslider_slide_eav_attribute';
		...
		$table = $this->setup->getConnection()->newTable(
			...
			'Mediarocks ProSlider Slide Eav Attribute'
		);
	 ...
	 */
	protected function createSlideEntityTable($entityCode, $type, $valueType, $valueLength = null) {
		...
		$table = $this->setup->getConnection()
			...
			->setComment('Mediarocks ProSlider Slide ' . $type . 'Attribute Backend Table');
		$this->setup->getConnection()->createTable($table);
	}
}

...
;(function (factory) {
  ...
  /** [Start For Custom Label] */
  $('.slider-container').append('<div class="custom_text"><span>New</span></div>');
  customLabel();
  function customLabel(){
    if ($('.label_skitter p').text() != '' ) {
      $('.custom_text span').text($('.label_skitter p').text());
      $('.custom_text').show();
    } else {
      $('.custom_text').hide();
    }
  }
  /** [End For Custom Label] */
  ...
  var defaults = {
    ...
    structure:    '<a href="#" class="prev_button">prev</a>'
                ...
                + '<div class="container_skitter">'
                  + '<div class="image">'
                    + '<a href=""><img class="image_main" alt="slider_images"/></a>'
  ...
  $sk.fn.extend({
      ...
      else
      {
        ...
        switch (self.settings.numbers_align) {
          case 'center' :
            var _vleft = (self.settings.width_skitter - self.skitter_box.find(class_info).outerWidth()) / 2;
            // self.skitter_box.find(class_info).css({'left': '50%', 'transform': 'translateX(-50%)'});
            self.skitter_box.find(class_info).css({'right': '0px', 'transform': 'translateX(-15%)'});
            break;
    ...
    addBoxClone: function(box_clone)
    {
      ...
      /** [Start: Change for the Banner link] */
      if ($(window).width() > 1000) {
        this.skitter_box.find('.image a').append(box_clone);
        this.skitter_box.find('.image a').css({"position": "absolute","width": "100%","left": "0"});
      } else {
        this.skitter_box.find('.container_skitter').append(box_clone);
      }
      /** [End: Change for the Banner link] */
    },

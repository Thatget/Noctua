...
class GridToXls extends Action
{
    ...
    public function execute()
    {
        ...
        $fileName = "FAQCategory(" . date('Y-m-d') . ").xls";
        ...
        $csvData = "Id,Category Name,Parent Category,Position,Status\n";
        foreach ($exportBlock as $key => $value) {
            $status = $value->getStatus() == "1" ? "Enabled" : "Disabled";
            $csvData .= $value->getId() . ',"' . $value->getCatName() . '","' . $value->getParentCatName() . '",' . $value->getPosition() . ',' . $status . '' . "\n";
        }

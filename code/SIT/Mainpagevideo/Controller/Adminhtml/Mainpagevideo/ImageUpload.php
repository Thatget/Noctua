...
class ImageUpload extends \Magento\Backend\App\Action
{
    ...
    {
        $imageUploadId = $this->_request->getParam('param_name', 'merchendise_image');
        try {
            $imageResult = $this->imageUploader->saveFileToTmpDir($imageUploadId);
            /**
             * Upload image folder wise : MD [START][27-11-2019]
             */
            $imageName = $imageResult['name'];
            $firstName = substr($imageName, 0, 1);
            $secondName = substr($imageName, 1, 1);

            $basePath = $this->filesystem->getDirectoryRead(DirectoryList::MEDIA)->getAbsolutePath() . 'merchendise/image/';
            $mediaRootDir = $this->filesystem->getDirectoryRead(DirectoryList::MEDIA)->getAbsolutePath() . 'merchendise/image/' . $firstName . '/' . $secondName . '/';

            if (!is_dir($mediaRootDir)) {
                $this->fileIo->mkdir($mediaRootDir, 0775);
            }
            /**
             * Set image name with new name, If image already exist [MD][27-11-2019]
             */
            $newImageName = $this->imageNewName($mediaRootDir, $imageName);
            $this->fileIo->mv($basePath . $imageName, $mediaRootDir . $newImageName);
            /**
             * Upload image folder wise : MD [END][27-11-2019]
             */
            $imageResult['cookie'] = [
            ...
            $imageResult['url'] = $mediaUrl . 'merchendise/image/' . $firstName . '/' . $secondName . '/' . $newImageName;
        } catch (\Exception $e) {

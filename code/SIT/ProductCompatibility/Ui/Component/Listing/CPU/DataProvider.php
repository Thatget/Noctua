 ...
 */
namespace SIT\ProductCompatibility\Ui\Component\Listing\CPU;
...
class DataProvider extends UiDataProvider
{
    ...
    protected function searchResultToOutput(SearchResultInterface $searchResult)
    {
        ...
        if (is_array($this->request->getParam('filters'))) {
            ...
            /**
             * Filter for series autocomplete
             */
            if (array_key_exists('comp_series', $this->request->getParam('filters'))) {
                $filterParams = $this->request->getParam('filters');
                $compSeriesLabel = $filterParams['comp_series'];
                $modelInfo = $this->prodCompHelper->getAttributeInfo(ProductCompHelper::COMP_EAV_TYPE, ProductCompHelper::COMP_SERIES);
                $seriesId = $modelInfo->getAttributeId();
                $manufAllOption = $this->prodCompHelper->getAttributeOptionAll($seriesId);
                $ids = $this->prodCompHelper->compareArrayValues($manufAllOption, $compSeriesLabel);
                $searchResult->addAttributeToFilter('comp_series', ['in' => $ids]);
            }
        }
        /**
         * Changes PP [15-5-2019] :  Product Name and Template Text Filter Logic in Ram Compatibility
    ...
    {
        /**
         * If you want to add filter which is custom column, then we need to add here as like product_name, template_text and comp_model, comp_socket, comp_manufacture, comp_series
         */
        if ($filter->getField() == 'product_name' || $filter->getField() == 'template_text' || $filter->getField() == 'comp_model' || $filter->getField() == 'comp_socket' || $filter->getField() == 'comp_manufacture' || $filter->getField() == 'comp_series') {
        } else {

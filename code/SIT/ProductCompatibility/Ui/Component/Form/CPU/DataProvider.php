 ...
 */
namespace SIT\ProductCompatibility\Ui\Component\Form\CPU;
	...
	public function getData() {
		if (!$this->loadedData) {
			...
			foreach ($items as $item) {
				...
				if ($item['comp_series'] != "") {
					$compSeriesAttrInfo = $this->prodCompHelper->getAttributeInfo(ProductCompHelper::COMP_EAV_TYPE, ProductCompHelper::COMP_SERIES);
					$compSeriesAttrId = $compSeriesAttrInfo->getAttributeId();
					$compSeriesoption = $this->prodCompHelper->getAttrOptionLabel($compSeriesAttrId, $item['comp_series'])->getFirstItem();
					$item['comp_series'] = $compSeriesoption['value'];
				}
				/**
				 * Changed by RH [24-04-2019] : Display data in mainboard form
				 */

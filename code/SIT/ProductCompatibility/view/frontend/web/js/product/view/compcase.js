define([
    ...
    return Component.extend({
        defaults: {
            template: 'SIT_ProductCompatibility/product/view/compcase',
        ...
        initialize: function(config) {
            ...
            var self = this;
            mainScope = self;
            if($("[data-id='tab-label-ccomp-title-content']").hasClass("active")){
                self.callAjax();
            }
            // Call ajax on click of "COMPATIBILITY BY CASE" tab
            $("[data-id='tab-label-ccomp-title-content']").click(function(){
                self.callAjax();
            });
            var currentUrl = history.state.Url;
            var str_pos = currentUrl.indexOf("ccomp#manuf_0");
            if (str_pos > -1) {
        ...
        getModel: function(manufacturer_id,model_id) {
            $(".service-case-comp .manufacture-case-name").removeClass('selected');
        ...
        getAllManufacturer: function() {
            $(".service-case-comp .manufacture-case-name").removeClass('selected');
            $('.case-all-data').addClass('selected');
            body = $('body').loader();
            body.loader('show');
            manufacturerId = 0;
        ...
        setGrid:function(){
            ...
            storage.post(
                ...
                function(response) {
                    ...
                    self.caseGrid(response);
                    body.loader('hide');
                    $('.case-bottom-content').show(); // enable bottom content after page load

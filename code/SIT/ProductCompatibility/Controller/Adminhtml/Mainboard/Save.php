	...
	public function execute() {
		...
		if ($data) {
			...
			$this->_eventManager->dispatch(
				'sit_productcompatibility_mainboard_prepare_save',
				['object' => $this->productcompFactory, 'request' => $this->getRequest()]
			);

			try {
				$productcompInstance->save();
				if (isset($data['productcompatibility_mainboard_products'])
					&& is_string($data['productcompatibility_mainboard_products'])) {
					$productsId = (array) json_decode($data['productcompatibility_mainboard_products']);

...
class GridToXls extends Action
{
    ...
    public function execute()
    {
        ...
        $fileName = "ProductCompatibility_" . $this->coreSession->getProCompType() . "(" . date('Y-m-d') . ").xls";
        ...
        $csvData = "entity_id,comp_socket,comp_manufacture,comp_model,comp_value,status,products,template_text\n";
        ...
        foreach ($exportBlock as $key => $value) {
            ...
            $csvData .= $value->getEntityId() . ',"' . $socketValue . '","' . $manufacturerValue . '","' . $modelValue . '","' . $compValue . '",' . $status . ',"' . $value->getPname() . '","' . $tempName . '"' . "\n";
        }

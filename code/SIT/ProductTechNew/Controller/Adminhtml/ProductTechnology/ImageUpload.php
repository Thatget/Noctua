...
class ImageUpload extends \Magento\Backend\App\Action
{
    ...
    {
        $imageUploadId = $this->_request->getParam('param_name', 'technology_image');
        try {
            $imageResult = $this->imageUploader->saveFileToTmpDir($imageUploadId);
            /**
             * Upload image folder wise : MD [START][19-03-2019]
             */
            $imageName = $imageResult['name'];
            $firstName = substr($imageName, 0, 1);
            $secondName = substr($imageName, 1, 1);

            $basePath = $this->filesystem->getDirectoryRead(DirectoryList::MEDIA)->getAbsolutePath() . 'producttechnology/image/';
            $mediaRootDir = $this->filesystem->getDirectoryRead(DirectoryList::MEDIA)->getAbsolutePath() . 'producttechnology/image/' . $firstName . '/' . $secondName . '/';
            ...
            /**
             * Upload image folder wise : MD [END][19-03-2019]
             */
            $imageResult['cookie'] = [
            ...
            $imageResult['url'] = $mediaUrl . 'producttechnology/image/' . $firstName . '/' . $secondName . '/' . $newImageName;
        } catch (\Exception $e) {

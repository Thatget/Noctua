...
class GridToXls extends Action
{
    ...
    public function execute()
    {
        ...
        $fileName = "GeneralTechnologies(" . date('Y-m-d') . ").xls";
        ...
        $csvData = "Id,Title,Status,URL Key\n";
        foreach ($exportBlock as $key => $value) {
            $status = $value->getStatus() == "1" ? "Enabled" : "Disabled";
            $csvData .= $value->getEntityId() . ',"' . $value->getGenTechnologyTitle() . '",' . $status . ',"' . $value->getUrlKey() . '"' . "\n";
        }

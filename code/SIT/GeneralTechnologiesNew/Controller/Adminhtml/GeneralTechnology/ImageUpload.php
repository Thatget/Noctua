...
class ImageUpload extends \Magento\Backend\App\Action
{
    ...
    {
        $imageUploadId = $this->_request->getParam('param_name', 'gen_technology_image');
        try {
            ...
            $basePath = $this->filesystem->getDirectoryRead(DirectoryList::MEDIA)->getAbsolutePath() . 'generaltechnology/image/';
            $mediaRootDir = $this->filesystem->getDirectoryRead(DirectoryList::MEDIA)->getAbsolutePath() . 'generaltechnology/image/' . $firstName . '/' . $secondName . '/';
            ...
            $imageResult['url'] = $mediaUrl . 'generaltechnology/image/' . $firstName . '/' . $secondName . '/' . $newImageName;
        } catch (\Exception $e) {

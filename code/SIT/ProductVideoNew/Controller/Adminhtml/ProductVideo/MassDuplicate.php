...
class MassDuplicate extends \Magento\Backend\App\Action {
	 ...
	 * @param \SIT\ProductVideoNew\Model\ProductVideoFactory $productFaq        [description]
	 * @param \SIT\ProductVideoNew\Model\ProductFactory       $product           [description]
	 */
	public function __construct(Context $context,
		Filter $filter,
		CollectionFactory $collectionFactory,
		\SIT\ProductVideoNew\Model\ProductVideoFactory $productFaq,
	...
	public function execute() {
		...
		foreach ($collection as $item) {
			...
			$products = $this->product->create()->getCollection()->addFieldToFilter("productvideo_id", ["eq" => $item->getEntityId()]);
	...
	private function insertProduct($productArray) {
		$videoProModel = $this->product->create();
		$videoProModel->setData($productArray);
		$videoProModel->save();
	}
}
